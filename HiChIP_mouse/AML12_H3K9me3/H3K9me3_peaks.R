##Analyse H3K9me3 HiChIP peaks intersected with RepeatMasker (peak_detection.sh script)
...
all = read_tsv('AML12-K9me3_norm-all_intersect_rm.txt.gz', col_names=cnames) %>% add_column(type='all')
uni = read_tsv('AML12-K9me3_norm-uni_intersect_rm.txt.gz', col_names=cnames) %>% add_column(type='uni')
...
ggplot(fam.peaks, aes(log10(uni+1), log2(all+1)-log2(uni+1))) +
	...
	ylab('log2 fold change')  #Figure 5D
...
##split pairs
sed -ne '1~8{N;N;N;p}' sim-${SGE_TASK_ID}.fq | gzip - > sim-${SGE_TASK_ID}-R1.fq.gz
sed -ne '5~8{N;N;N;p}' sim-${SGE_TASK_ID}.fq | gzip - > sim-${SGE_TASK_ID}-R2.fq.gz
rm sim-${SGE_TASK_ID}.fq

##make bed file
module unload python/2.7.15
module load python/3.6.3
python get_original_pos.py -r1 sim-${SGE_TASK_ID}-R1.fq -r2 sim-${SGE_TASK_ID}-R2.fq

##align
. ~/PAtChER/patcherenv/bin/activate
patcher -g hg38.fa -r1 sim-${SGE_TASK_ID}-R1.fq.gz -r2 sim-${SGE_TASK_ID}-R2.fq.gz -d 20000 -o sim-${SGE_TASK_ID}.bam -b
